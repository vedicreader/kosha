...
karma is a **dev dependency** — it runs at development time so your AI
...
# pip
pip install --group dev karma
```

## One-time project setup

Run this once to drop a `SKILL.md` into `.agents/skills/karma/` — the
file your AI harness reads to know karma exists and how to call it.

``` python
from karma import Karma

Karma(install_skill=True)   # writes .agents/skills/karma/SKILL.md at your repo root
...
``` python
k = Karma()   # auto-detects git repo root

k.sync(pkgs=['fasthtml', 'fastcore', 'litesearch'])
# Indexes:
#   .karma/code.db   — your repo code chunks + embeddings
#   .karma/graph.db  — call graph (callers, callees, PageRank)
#   ~/.local/share/karma/env.db — installed packages (shared across repos)
```
...
`k.graph` is a
[`CodeGraph`](https://vedicreader.github.io/karma/graph.html#codegraph)
backed by `.karma/graph.db`. After `k.sync()`, the graph covers your
...
The `Karma(install_skill=True)` call above writes
`.agents/skills/karma/SKILL.md`. Most agent harnesses (Claude Code,
...
Once installed globally, Claude Code will load the karma skill at the
start of every session in every repo.

### Other harnesses

Place `SKILL.md` wherever the harness discovers agent skills. Common
locations: - `.agents/skills/karma/SKILL.md` — general convention -
`.continue/skills/karma/SKILL.md` — Continue.dev - Configure in harness
