...
Docs: https://vedicreader.github.io/koshagraph.html.md"""
...
# %% ../nbs/01_graph.ipynb #71972d4335da4760
@patch
def digest(self: Kosha, pkg: str = None, top_k: int = 15, dep_depth: int = 2, out: Path = None) -> str:
	"Generate a compact LLM-ready codebase passport, written to .kosha/DIGEST.md"
	from datetime import date
	lines = []

	# Header
	st = self.status()
	pkg = pkg or self.root.name
	lines += [f"# Codebase Digest: {self.root.name}", f"Generated: {date.today()}  |  Root: `{self.root}`",
			   f"files: {st['files']}  graph_nodes: {st['graph_nodes']}  packages: {st['packages']}", ""]

	# Module Architecture — group top nodes by module prefix
	top_all = self.graph.ranked(k=100)
	seen_mods, mod_order = {}, []
	for row in top_all:
		parts = row['node'].split('.')
		mod = '.'.join(parts[:-1]) if len(parts) >= 2 else parts[0]
		if mod not in seen_mods: seen_mods[mod] = row['node']; mod_order.append(mod)
	if mod_order:
		lines += ["## Module Architecture"]
		for mod in mod_order[:12]:
			rows = self.code_st(select='metadata', where="json_extract(metadata,'$.mod_name')=?", where_args=[seen_mods[mod]], limit=1)
			doc = ''
			if rows:
				m = jl(rows[0]['metadata']) if isinstance(rows[0].get('metadata'), str) else rows[0].get('metadata', {})
				doc = (m.get('docstring') or '').split('\n')[0].strip()[:80]
			lines.append(f"- **{mod}** — {doc}" if doc else f"- **{mod}**")
		lines.append("")

	# Top critical nodes
	top_nodes_r = self.graph.ranked(k=top_k)
	if top_nodes_r:
		lines += [f"## Top {top_k} Critical Nodes (by PageRank)"]
		for i, row in enumerate(top_nodes_r, 1):
			node, pr = row['node'], row['pagerank']
			nr = self.gn(select='in_degree,out_degree', where='node=?', where_args=[node])
			ind, outd = (nr[0]['in_degree'], nr[0]['out_degree']) if nr else ('?', '?')
			lines.append(f"{i}. {node}  (pr: {pr:.5f}  in: {ind}  out: {outd})")
		lines.append("")

	# Public API
	api = self.public_api(pkg, meta_cols='mod_name,docstring', limit=30)
	if api:
		lines += [f"## Public API: {pkg}"]
		for r in api:
			name = r.get('mod_name', '')
	...
	# Dependency layers
	try:
		layers = self.dep_stack(seeds=[pkg], depth=dep_depth)
		if layers:
			lines += ["## Dependency Layers"]
			for i, layer in enumerate(layers):
				label = "Seeds" if i == 0 else f"Depth {i}"
				items = ', '.join(sorted(layer) if isinstance(layer, (set, frozenset)) else layer)
	...
	try:
		kinds = self.graphdb.execute("SELECT kind, COUNT(*) n FROM graph_edges GROUP BY kind ORDER BY n DESC").fetchall()
		if kinds:
			lines += ["## Graph Edge Patterns"]
			lines.append("  ".join(f"{k}: {n}" for k, n in kinds))
			lines.append("")
	except Exception: pass

	out_path = Path(out) if out else self.root / '.kosha' / 'DIGEST.md'
