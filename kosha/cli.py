...
Docs: https://vedicreader.github.io/koshacli.html.md"""
...
__all__ = ['CMDS', 'sync', 'context', 'repo_context', 'env_context', 'ni', 'watch', 'public_api', 'api_paths', 'dep_stack',
           'top_nodes', 'status', 'where_to_add', 'daemon', 'install', 'digest', 'main']
...
# %% ../nbs/03_cli.ipynb #6706706f29984652
@call_parse
def digest(
    pkg:str=None,       # package for public API focus (defaults to repo name)
    top_k:int=15,       # number of top critical nodes to include
    dep_depth:int=2,    # BFS depth for dependency layers
    out:str=None,       # output path (defaults to .kosha/DIGEST.md)
    quiet:bool=False,   # suppress stdout output
):
    "Generate a compact LLM-ready codebase passport written to .kosha/DIGEST.md"
    k = Kosha()
    result = k.digest(pkg=pkg, top_k=top_k, dep_depth=dep_depth, out=out)
