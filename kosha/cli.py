...
__all__ = ['CMDS', 'sync', 'context', 'repo_context', 'env_context', 'ni', 'watch', 'public_api', 'api_paths', 'dep_stack',
           'top_nodes', 'status', 'where_to_add', 'find_related', 'savings', 'bench', 'mcp', 'daemon', 'install',
           'main']
...
# %% ../nbs/03_cli.ipynb #cli-semble-cmds
@call_parse
def find_related(
    path:str,           # file path (absolute or repo-relative)
    line:int=None,      # line number the chunk should cover
    limit:int=10,       # max related chunks
    as_json:bool=False, # output JSON
):
    "Chunks semantically similar to the code at path:line, plus its call-graph neighborhood."
    r = Kosha().find_related(path, line=line, limit=limit)
    if as_json: print(json.dumps(_to_json(r)))
    else:
        s = r.get('source') or {}
        print(f"\n## related to {s.get('mod_name') or path}:{line or s.get('lineno','?')}")
        if r.get('error'): print(f"  {r['error']}"); return
        for x in r.get('related', []):
            m = x.get('metadata') or {}
            where = m.get('mod_name') or m.get('path') or '?'
            print(f"  {where}  L{m.get('lineno','?')}  dist={x.get('_dist', 0):.3f}  [{x.get('_store','')}]")
        for kk in ('callers','callees','co_dispatched'):
            if r.get(kk): print(f"{kk}: {', '.join(list(r[kk])[:6])}")

@call_parse
def savings(
    as_json:bool=False, # output JSON
):
    "Cumulative tokens saved vs reading matched files in full (conservative, chars/4)."
    s = Kosha().savings()
    if as_json: print(json.dumps(_to_json(s)))
    else:
        a = s['all_time']
        print(f"tokens saved — all time: {a['tokens_saved']:,} ({a['pct_saved']}% smaller than reading files, {a['calls']} calls)")
        print(f"  today: {s['today']['tokens_saved']:,}   last 7d: {s['last_7d']['tokens_saved']:,}")
        for c, v in s.get('by_call', {}).items(): print(f"  {c}: {v['tokens_saved']:,} across {v['calls']} calls")

@call_parse
def bench(
    n:int=50,           # number of eval queries
    limit:int=10,       # retrieval depth
    store:str='code',   # sample queries from: code | env
    compare:bool=False, # also run with rerank off and report both
    as_json:bool=False, # output compact JSON
):
    "Self-retrieval benchmark on the current index (relative numbers — see kosha.bench docs)."
    from kosha.bench import sample_queries, run_bench, compare_rerank
    k = Kosha()
    if compare: r = compare_rerank(k, n=n, store=store)
    else: r = run_bench(k, sample_queries(k, store=store, n=n), limit=limit)
    print(json.dumps(_to_json(r)) if as_json else json.dumps(_to_json(r), indent=2))

@call_parse
def mcp():
    "Run kosha as an MCP (Model Context Protocol) stdio server."
    from kosha.mcp import serve
    serve()
...
_DISPATCH = {
    ...
    'find_related':   lambda k, a: k.find_related(**a),
    'savings':        lambda k, a: k.savings(),
}
...
@call_parse
def install(
    mcp:bool=True,  # also write MCP server config (.mcp.json for Claude Code, .cursor/mcp.json for Cursor)
):
    "Install kosha SKILL.md to .agents/skills/kosha/ and .claude/skills/kosha/, plus MCP server config."
    from kosha.core import mv_skill_md, repo_root
    mv_skill_md(dry_run=False, dir=repo_root())
    if mcp:
        from kosha.mcp import install_mcp
        install_mcp(repo_root())
