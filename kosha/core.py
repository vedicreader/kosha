...
# %% auto #0
__all__ = ['strict_skip_folder_re', 'strict_skip_file_re', 'parse', 'repo_root', 'mv_skill_md', 'arun', 'pkg_trans_deps',
...
def update_repo(self:Kosha,
    ...
    if to_remove:
        self.code_st.delete_where(where=f'path in ({",".join(to_remove.map(repr))})')
