...
# %% auto #0
__all__ = ['cr_instr', 'model', 'static_model', 'strict_skip_folder_re', 'strict_skip_file_re', 'parse', 'repo_root',
           'mv_skill_md', 'arun', 'pkg_trans_deps', 'env_pkg_versions', 'embedder', 'static_embedder', 'emb_doc',
           'emb_query', 'Kosha', 'pkg_url', 'pkg_doc', 'has_init', 'imp_root', 'enrich_chunks', 'embed_chunk',
           'process_content', 'count_imp', 'ignored', 'parseq', 'filt2wh']

# %% ../nbs/00_core.ipynb #d979fe142033f158
import ast, os, re, time, subprocess, fnmatch
import numpy as np
...
from .rank import expand_query, chunk_stems
...
static_model = 'minishlab/potion-code-16M'
...
@timed_cache(3600*24)
def static_embedder(
    emb_model=static_model  # HuggingFace model id for static embeddings
) -> 'BaseEmbeddings':
    'Load or return cached static (potion) embedder (24h TTL). CPU-only, no transformer inference.'
...
def emb_doc(
	...
	'Curry an embedder factory into a document-embedding function `t,**kw -> embeddings`. Static output is cast to float16 to match FastEncode and vec_search.'
	def f(t, **kw):
		enc = e()
		if isinstance(enc, FastEncode): return enc.encode_document(t, **kw)
		return [np.asarray(v, dtype=np.float16) for v in enc.embed_batch(listify(t))]
	return f
...
def emb_query(
	...
	def f(t, **kw):
		enc = e()
		if isinstance(enc, FastEncode): return enc.encode_query(t, **kw)
		return np.asarray(enc.embed_batch(listify(t))[0], dtype=np.float16)
	return f

# %% ../nbs/00_core.ipynb #b18bbd41de289e55
def _kv_table(db):
	'key/value metadata table for a kosha database (embedding model id, schema flags).'
	t = db.t.kosha_meta
	t.create(key=str, value=str, pk='key', if_not_exists=True)
	return t

class Kosha:
	'''Kosha allows you to build a context for code generation based on your repo and environment.
	profile: 'fast' (default) = static potion-code embeddings, millisecond CPU indexing/query;
	'accurate' = CodeRankEmbed ONNX transformer. Override via KOSHA_PROFILE. Changing profile
	requires re-embedding: k.sync(force=True).'''
	def __init__(self, dir: Path = None, install_skill: bool = False, xdg_dir: Path = None, efn=None,
	             profile: str = None, db_dir: Path = None):
		self.root, self.xdg = Path(dir or repo_root() or '.'), xdg_dir or xdg_data_home()
		self.profile = (profile or os.environ.get('KOSHA_PROFILE') or 'fast').lower()
		if efn is not None: self.efn, self.emb_model = efn, 'custom'
		elif self.profile == 'accurate': self.efn, self.emb_model = bind(embedder, md=self.xdg/'model'), model.model
		else: self.efn, self.emb_model = static_embedder, static_model
		self.emb_doc, self.emb_query = emb_doc(self.efn), emb_query(self.efn)
		if install_skill: mv_skill_md(dir=self.root, dry_run=False)
		self.cp, self.ce = Path(db_dir or self.root/'.kosha')/'code.db', self.xdg.joinpath('kosha','env.db')
		...
		self.code_kv, self.env_kv = _kv_table(self.codedb), _kv_table(self.envdb)
		self.sv = self.envdb.t.savings
		self.sv.create(id=int, ts=float, call=str, files=int, file_chars=int, snippet_chars=int,
		               pk='id', if_not_exists=True)
		self._warn_emb_mismatch()

	def _warn_emb_mismatch(self):
		'Warn when an existing index was embedded with a different model than the current profile uses.'
		if self.emb_model == 'custom': return
		for kv, nm in ((self.code_kv,'code.db'), (self.env_kv,'env.db')):
			cur = first(kv(where='key=?', where_args=['emb_model']))
			if cur and cur['value'] != self.emb_model:
				print(f"WARNING: {nm} embeddings were built with {cur['value']!r} but profile {self.profile!r} "
				      f"uses {self.emb_model!r}. Re-embed with k.sync(force=True), or set KOSHA_PROFILE to match.")
...
def enrich_chunks(content: L) -> L:
	...
	out = enriched + enriched.filter(lambda d: d['metadata'].get('type')=='ClassDef').map(_cls_methods).concat()
	return out.map(lambda d: d | {'metadata': d['metadata'] | dict(stems=chunk_stems(d['metadata']))})
...
def process_env(self:Kosha, content=None, reembed=False, **kwargs):
	...
	self.env_kv.insert(dict(key='emb_model', value=self.emb_model), replace=True)
...
# %% ../nbs/00_core.ipynb #core-ignored
def _ignore_pats(root) -> list:
	'Patterns from .koshaignore (gitignore-ish: # comments and blank lines skipped, dir/ suffix supported).'
	f = Path(root)/'.koshaignore'
	if not f.exists(): return []
	return [l.strip() for l in f.read_text().splitlines() if l.strip() and not l.startswith('#')]

def ignored(files, root) -> set:
	'Subset of files excluded by git check-ignore (when root is a git repo) or .koshaignore patterns.'
	fs, root = list(map(str, files)), str(root)
	out = set()
	if fs and (Path(root)/'.git').exists():
		try:
			p = subprocess.run(['git','-C',root,'check-ignore','--stdin'], input='\n'.join(fs),
			                   capture_output=True, text=True, timeout=15)
			out |= set(p.stdout.splitlines())
		except Exception: pass
	if pats := _ignore_pats(root):
		for f in fs:
			rel = os.path.relpath(f, root)
			if any(fnmatch.fnmatch(rel, p) or fnmatch.fnmatch(Path(f).name, p) or
			       fnmatch.fnmatch(rel, p.rstrip('/') + '/*') for p in pats): out.add(f)
	return out
...
def process_repo(self:Kosha, content=None, reembed=False, **kwargs):
	...
	self.code_kv.insert(dict(key='emb_model', value=self.emb_model), replace=True)
...
def update_repo(self:Kosha,
	...
	if ig := ignored(all_files, dir): all_files = L(all_files).filter(lambda f: str(f) not in ig)
...
def env_context(self:Kosha,
				...
				sys_wide=True,
				log:bool=True,			# record token-savings telemetry for this call
				**kw					# additional args to pass to db.search
				) -> L:
	'Code search through the database to find relevant code snippets.'
	raw, fs = parseq(q)
	fq, emb = pre(expand_query(raw), wide=wide, extract_kw=False), self.emb_query(emb_q or raw)
	...
	out = L(self.envdb.search(fq, emb.tobytes(), columns.split(','), where=wh, **kw)).map(fn)
	if log: self._log_savings('env_context', out)
	return out
...
def repo_context(self:Kosha,
	...
	fn = lambda r: r | dict(metadata=jl(r['metadata'])) if 'metadata' in r else r
	out = L(self.codedb.search(fq, emb.tobytes(), columns.split(','), where=wh, **kw)).map(fn)
	if log: self._log_savings('repo_context', out)
	return out
...
# %% ../nbs/00_core.ipynb #core-savings
@patch
def _log_savings(self:Kosha, call:str, results):
	'Record token-savings telemetry: total chars of the unique files behind the results vs chars returned.'
	if os.environ.get('KOSHA_SAVINGS','1') == '0': return
	try:
		rs = [r for r in (results or []) if isinstance(r, dict)]
		_m = lambda r: r.get('metadata') if isinstance(r.get('metadata'), dict) else {}
		paths = {str(p) for r in rs if (p := _m(r).get('path') or r.get('path'))}
		fc = sum(p.stat().st_size for p in map(Path, paths) if p.exists())
		sc = sum(len(r.get('content') or '') for r in rs)
		self.sv.insert(dict(ts=time.time(), call=call, files=len(paths), file_chars=fc, snippet_chars=sc))
	except Exception: pass

@patch
def savings(self:Kosha) -> dict:
	'Cumulative tokens saved vs reading matched files in full ((file_chars - snippet_chars)/4, conservative).'
	def agg(where=None):
		r = first(self.envdb.q('SELECT COUNT(*) n, COALESCE(SUM(file_chars),0) fc, COALESCE(SUM(snippet_chars),0) sc '
		                       f"FROM savings{' WHERE ' + where if where else ''}"))
		return dict(calls=r['n'], tokens_saved=max(0, r['fc'] - r['sc'])//4,
		            pct_saved=round(100*(1 - r['sc']/r['fc']), 1) if r['fc'] else 0.0)
	now = time.time()
	by_call = {r['call']: dict(calls=r['n'], tokens_saved=max(0, (r['fc'] or 0) - (r['sc'] or 0))//4)
	           for r in self.envdb.q('SELECT call, COUNT(*) n, SUM(file_chars) fc, SUM(snippet_chars) sc '
	                                 'FROM savings GROUP BY call')}
	return dict(all_time=agg(), today=agg(f'ts >= {now - 86400}'), last_7d=agg(f'ts >= {now - 7*86400}'),
	            by_call=by_call)

_last_auto = {}
@patch
def maybe_sync(self:Kosha, min_interval:float=30.0):
	'Incremental repo re-index if anything changed, throttled per root. Used by context() auto-sync.'
	key = str(self.root)
	if time.time() - _last_auto.get(key, 0) < min_interval: return
	_last_auto[key] = time.time()
	try:
		self.update_repo(verbose=False)
		self.graph.sync_dir(self.root)
	except Exception: pass
