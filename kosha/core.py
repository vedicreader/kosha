...
__all__ = ['cr_instr', 'model', 'strict_skip_folder_re', 'strict_skip_file_re', 'parse', 'repo_root', 'mv_skill_md', 'arun',
           'pkg_trans_deps', 'env_pkg_versions', 'embedder', 'static_embedder', 'emb_doc', 'emb_query', 'AnnIndex',
           'ann_sync', 'ann_search', 'Kosha', 'pkg_url', 'pkg_doc', 'has_init', 'imp_root', 'enrich_chunks',
...
# %% ../nbs/00_core.ipynb #9e1ca92568e09d5d
class AnnIndex:
	'Persistent usearch HNSW index keyed by sqlite rowid; created lazily on first add (ndim from the vectors).'
	def __init__(self,
				 path,          # index file location, loaded if it exists
				 metric='cos',  # usearch distance metric
				 dtype='f16',   # usearch storage dtype (matches FastEncode float16 output)
	):
		self.path, self.metric, self.dtype, self.idx = Path(path), metric, dtype, None
		if self.path.exists():
			from usearch.index import Index
			self.idx = Index.restore(str(self.path))
	def __len__(self): return len(self.idx) if self.idx is not None else 0
	def _keys(self, keys): return np.asarray(list(keys), dtype=np.uint64)
	def add(self, keys, vecs):
		'Insert or overwrite vectors for keys, then persist to disk.'
		if not len(vecs): return
		if self.idx is None:
			from usearch.index import Index
			self.idx = Index(ndim=vecs.shape[-1], metric=self.metric, dtype=self.dtype)
		ks = self._keys(keys)
		self.idx.remove(ks)
		self.idx.add(ks, vecs)
		self.save()
	def remove(self, keys):
		'Drop keys from the index (missing keys are ignored), then persist.'
		if self.idx is None or not len(ks := self._keys(keys)): return
		self.idx.remove(ks)
		self.save()
	def contains(self, keys):
		'Bool array marking which keys are already indexed.'
		ks = self._keys(keys)
		return self.idx.contains(ks) if self.idx is not None else np.zeros(len(ks), bool)
	def search(self, vec, k=50) -> list:
		'Approximate nearest neighbours of vec: list of (rowid, distance), best first.'
		if not len(self): return []
		m = self.idx.search(np.asarray(vec).reshape(-1), k)
		return list(zip(map(int, m.keys), map(float, m.distances)))
	def save(self):
		self.path.parent.mkdir(parents=True, exist_ok=True)
		if self.idx is not None: self.idx.save(str(self.path))

# %% ../nbs/00_core.ipynb #6307cc63f007eb6a
def ann_sync(store, idx: AnnIndex, dtype=np.float16) -> int:
	'Move embeddings from a sqlite store into the ANN index (keyed by rowid) and clear the column. Returns rows moved.'
	rows = store.db.q(f'select rowid, embedding from {store.name} where embedding is not null')
	if not rows: return 0
	idx.add([r['rowid'] for r in rows], np.vstack([np.frombuffer(r['embedding'], dtype=dtype) for r in rows]))
	store.db.q(f'update {store.name} set embedding=NULL where embedding is not null')
	return len(rows)

def ann_search(store,                # litesearch store (Table) holding content + FTS5
			   idx: AnnIndex,        # ANN index with the store vectors, keyed by rowid
			   q: str,               # FTS query string
			   emb: bytes,           # query embedding vector (as bytes)
			   columns: list = None, # columns to return
			   where: str = None,    # additional where clause (applied to FTS and ANN hits)
			   ...
			   rrf=True,             # merge FTS + ANN results with reciprocal rank fusion
			   rrf_k: int = 60,      # RRF k parameter
			   dtype=np.float16,     # query embedding dtype
			   oversample: int = 4,  # ANN over-fetch factor to survive post-filtering
			   quote: bool = False,  # quote FTS query to disable special chars
			   ):
	'Approximate counterpart of litesearch `Database.search`: FTS5 + usearch ANN combined with RRF.'
	if not q.strip(): return None
	db, lim = store.db, (limit or 50) + (offset or 0)
	cols = list(columns or [])
	if 'rowid' not in cols: cols = ['rowid'] + cols
	sql = store.search_sql(order_by='rank', columns=cols, limit=lim, where=where, include_rank=True)
	fts = db.q(sql, merge(dict(query=db.quote_fts(q) if quote else q), where_args or {}))
	hits, vec = idx.search(np.frombuffer(emb, dtype=dtype), lim*oversample), []
	if hits:
		sel = ','.join(f'{c} as {c}' if c == 'rowid' else c for c in cols)
		wh = 'rowid in (%s)' % ','.join(str(k) for k,_ in hits) + (f' AND ({where})' if where else '')
		rows = {r['rowid']: r for r in db.q(f'select {sel} from {store.name} where {wh}', where_args or {})}
		if not where: idx.remove([k for k,_ in hits if k not in rows])  # rows deleted from the store
		vec = [rows[k] | {'_dist': d} for k,d in hits if k in rows][:lim]
	if rrf: return rrf_merge(fts, vec, k=rrf_k, limit=lim)[offset or 0:]
...
class Kosha:
	...
	def __init__(self, dir: Path = None, install_skill: bool = False, xdg_dir: Path = None, efn=embedder,
				 exact: bool = True):  # exact brute-force vector scan in sqlite; False = approximate usearch HNSW index
		self.root, self.xdg, self.exact = Path(dir or repo_root() or '.'), xdg_dir or xdg_data_home(), exact
		...
		self.codedb, self.envdb = database(self.cp, sem_search=exact), database(self.ce, sem_search=exact)
		...
		self.ann = {} if exact else {st: AnnIndex(p.parent/f'{p.stem}.{st.name}.usearch')
									 for st,p in ((self.code_st,self.cp),(self.env_st,self.ce),(self.pkg_st,self.ce))}
		for st,idx in self.ann.items(): ann_sync(st, idx)

	def _ann(self, store) -> 'AnnIndex | None':
		'usearch index backing a store, or None in exact mode.'
		return self.ann.get(store)
...
@patch
def _unembedded(self:Kosha, store, reembed=False) -> L:
	'Rows still needing a vector: everything if reembed, else NULL-embedding rows not yet in the ANN index.'
	if reembed: return L(store())
	if self.exact: return L(store(where='embedding is NULL'))
	rows = L(store(select='rowid, *', where='embedding is NULL'))
	if not rows: return rows
	has = self._ann(store).contains(rows.attrgot('rowid'))
	return L(r for r,h in zip(rows, has) if not h).map(lambda r: filter_keys(r, not_(in_(('rowid',)))))
...
def process_env(self:Kosha, content=None, reembed=False, **kwargs):
	...
	res = process_content(self.env_st, content or self._unembedded(self.env_st, reembed), emb_fn=self.emb_doc, **kwargs)
	if not self.exact: ann_sync(self.env_st, self._ann(self.env_st))
	return res
...
def process_repo(self:Kosha, content=None, reembed=False, **kwargs):
	...
	res = process_content(self.code_st, content or self._unembedded(self.code_st, reembed), emb_fn=self.emb_doc, **kwargs)
	if not self.exact: ann_sync(self.code_st, self._ann(self.code_st))
	return res
...
# %% ../nbs/00_core.ipynb #104531d7f522dd08
@patch
def search(self:Kosha, store, q, emb, columns=None, where=None, **kw):
	'Hybrid FTS+vector search on a store: exact sqlite scan by default, usearch ANN + RRF when exact=False.'
	if self.exact: return store.db.search(q, emb, columns, where=where, table_name=store.name, **kw)
	return ann_search(store, self._ann(store), q, emb, columns=columns, where=where, **kw)
...
def env_context(self:Kosha,
	...
	return L(self.search(self.env_st, fq, emb.tobytes(), columns.split(','), where=wh, **kw)).map(fn)
...
def repo_context(self:Kosha,
	...
	fn = lambda r: r | dict(metadata=jl(r['metadata'])) if 'metadata' in r else r
	return L(self.search(self.code_st, fq, emb.tobytes(), columns.split(','), where=wh, **kw)).map(fn)
...
def pkg_context(self:Kosha,
	...
	return L(self.search(self.pkg_st, fq, emb, columns=['content','metadata'], limit=limit)).map(fn)
